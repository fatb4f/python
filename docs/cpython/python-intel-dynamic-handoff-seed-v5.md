# Handoff Seed v5 — Python-Intel Arrow-Backed Execution Facts and Semantic Policy
...
Dynamic orchestration is best modeled as **semantic query/probe lowering over an observed execution graph**. Rust owns graph reconstruction, physical-store admission, query planning, intent validation, effects, and causal joins. CPython supplies compiler/runtime observations. Regrtest supplies campaign mechanics and orchestration facts. Arrow may store normalized accepted facts. Steel may compute policy over immutable Rust semantic snapshots but cannot execute effects.
...
Do not place semantic authority or effect execution in Marimo, Pydantic Graph, pytest, generated Python, regrtest, JSONata, Arrow, or Steel.
...
The currently evaluated release is 59.2.0, licensed Apache-2.0 with Rust 1.85 minimum support. Prefer the narrow `arrow-array`, `arrow-schema`, and `arrow-ipc` crates with unnecessary default features disabled; do not add DataFusion, Parquet, CSV, JSON, FFI, or PyArrow features to E8.
...
## 3.6 Steel — semantic query/policy candidate
...
Until this gate passes and promotion is recorded, Arrow remains optional dual-write evaluation infrastructure. DataFusion, Parquet, worker-side Arrow generation, and direct Steel access to Arrow are out of scope.
...
If E1, E2, E3, and E6 have concrete answers, the causal semantic architecture is established. E8 then determines whether Arrow is promoted from dual-write candidate to the default physical fact store; it has implementation priority over E5 and E7. E5 qualifies only external-JSON adapter projection and does not replace typed normalization or causal validation. E7 qualifies optional embedded policy but does not grant storage or execution authority. E4 qualifies campaign reduction rather than gating the initial graph prototype.
